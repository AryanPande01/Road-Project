...
def process():
    ...
    zone = request.form.get("zone", temp2.config.DEFAULT_ZONE).strip()
    rate_input = request.form.get("rate")
    rate = _float(rate_input, temp2.config.get_zone_rate(zone)) if rate_input else temp2.config.get_zone_rate(zone)
    ...
    try:
        if mode == "text":
            ...
            data, boq = textmode.build_text_kmz(
                points, text, rate=rate, zone=zone, log=lambda *_: None,
    ...
    return jsonify(
        id=rid, mode=mode, name=base, points=len(points), zone=zone,
...
def generate_boq():
    ...
    # Corridor mode when the KML uses 'x' delimiters (Task 5): every placemark
    # between two x's shares a tab, with an accurate per-item road width (Task
    # 4). With no x's, keep the original geographic-patch grouping so existing
    # inputs behave exactly as before.
    corridor_mode = temp2.has_corridor_delimiters(points)
    try:
        if corridor_mode:
            grouped = temp2.build_corridor_boq(points, zone=zone, log=lambda *_: None)
            buf = io.BytesIO()
            temp2.export_corridor_excel(grouped, buf)
        else:
            grouped = temp2.build_grouped_boq(points, zone=zone)
            buf = io.BytesIO()
    ...
    # Per-group preview for the UI (patches or corridors, same shape).
    patches = [{
        "patch": label,
        "items": rows,
    ...
    return jsonify(
        id=rid, name=base, points=len(points), zone=zone, patches=patches,
        grouping=("corridor" if corridor_mode else "patch"),
        download_url=f"/download/{rid}",
        download_name=f"BOQ_{base}_{zone}.xlsx",
    )


@app.route("/zone_prices", methods=["GET", "POST"])
def zone_prices():
    """Read or update the paint price per zone (Task 1).

    GET  -> the effective price for every zone (defaults + any overrides).
    POST -> apply a {zone: price} mapping of user-entered prices, then return
            the new effective prices. Validation lives in config so the same
            rules apply everywhere; a bad value leaves all prices unchanged.
    """
    if request.method == "GET":
        return jsonify(rates=temp2.config.get_all_zone_rates(),
                       default_zone=temp2.config.DEFAULT_ZONE)

    payload = request.get_json(silent=True) or {}
    mapping = payload.get("rates", payload)
    try:
        rates = temp2.config.set_zone_rates(mapping)
    except ValueError as e:
...
def boq(rid):
    ...
    if r["boq"] and "_corners" in r["boq"][0] and not fast_mode:
