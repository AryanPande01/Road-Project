...
def fetch_multi_bbox_roads(bboxes, offline=False):
    """Return OSM highway 'way' elements across multiple bboxes in an optimized Overpass query."""
    if not bboxes:
        return []
    ...
    # Caching disabled by user request - always fetch fresh
    ...
    for mirror in OVERPASS_MIRRORS:
        for attempt in range(2):
            try:
                r = s.post(mirror, data={"data": query}, timeout=15)
                if r.status_code == 200:
                    ...
                    print(f"    OSM: {len(unique_els)} unique ways from {urllib.parse.urlparse(mirror).netloc}")
                    return unique_els
                else:
                    print(f"    Overpass {urllib.parse.urlparse(mirror).netloc} returned HTTP {r.status_code}")
            except Exception as e:
...
def make_elements_fetcher(points, offline=False, log=print):
    ...
    shared_elements = fetch_multi_bbox_roads(bboxes, offline=offline)
    ...
    def _process_one_image_point(item_tuple):
        ...
        if known and fname:
            ...
            ring_lonlat, corners_xy = _straight_strip(
                frame, foot, tan, hw, hw, mark_length)
