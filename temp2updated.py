...
OUTPUT_DIR    = "outputs"
# CACHE_DIR     = "cache"  # Caching disabled by user request
    ...
    if _HTTP_SESSION is None:
        with _HTTP_SESSION_LOCK:
            if _HTTP_SESSION is None:
                s = requests.Session()
                s.headers.update({
                    "User-Agent": "RoadMarkStudio/5.0 (Windows NT 10.0; Win64; x64) OSM-Road-Client",
                    "Accept": "application/json, */*"
                })
                adapter = requests.adapters.HTTPAdapter(pool_connections=25, pool_maxsize=50, max_retries=2)
...
def get_satellite_image(corners):
    ...
    Caching is disabled by user request.
    """
    ...
    # cache_key = f"{cx:.6f}_{cy:.6f}_{hashlib.sha1(str(corners).encode()).hexdigest()[:8]}"
    # cache_path = os.path.join(SCREENSHOTS_DIR, f"sat_{cache_key}.png")
...
# ==================================================================
# Overpass fetch  (requests with connection pool, mirror fallback)
# ==================================================================
#
# The Overpass road query is the single largest cost in KML generation (a slow
# mirror can take 10-15 s). The road network for a given area is static over a
# session, and BOTH the image-KMZ step and the BOQ step query the same bbox, so
# we cache the result in memory keyed by the exact query. This turns every
# repeat (re-runs, the KMZ->BOQ handoff) into an instant lookup instead of a
# fresh network round-trip. Single-flight locking stops concurrent callers from
# firing duplicate queries for the same area.

_OSM_CACHE = {}                    # query string -> list[element]
_OSM_CACHE_LOCK = threading.Lock()
_OSM_EVENTS = {}                   # query string -> threading.Event (in-flight)


def clear_osm_cache():
    """Drop the cached Overpass responses (benchmarks / long-running servers)."""
...
def fetch_bbox_roads(south, west, north, east, offline=False):
    """Return OSM highway 'way' elements in the bbox."""
    return fetch_multi_bbox_roads([(south, west, north, east)], offline=offline)


def _overpass_one(mirror, query):
    """POST the query to one mirror; return deduped ways on HTTP 200, else None."""
    r = get_http_session().post(mirror, data={"data": query}, timeout=12)
...
def _overpass_request(query, log):
    """Query Overpass, racing the mirrors concurrently and returning the first
    successful response. Overpass mirrors vary from ~1 s to timing out, so
    racing them (instead of trying one, waiting, then the next) cuts the cold
    fetch to the fastest mirror and survives a dead primary. Returns deduped
    ways, or [] if every mirror fails."""
    from concurrent.futures import ThreadPoolExecutor, as_completed
    # Race a small batch of mirrors first (fast + spreads load across servers
    # rather than hammering one); fall back to the rest only if that batch all
    # fails, so we stay polite to the free Overpass infrastructure.
    for batch in (OVERPASS_MIRRORS[:3], OVERPASS_MIRRORS[3:]):
        if not batch:
            continue
        with ThreadPoolExecutor(max_workers=len(batch)) as ex:
            futs = {ex.submit(_overpass_one, m, query): m for m in batch}
            for fut in as_completed(futs):
                mirror = futs[fut]
                try:
                    unique = fut.result()
                except Exception as e:
                    log(f"    Overpass {urllib.parse.urlparse(mirror).netloc} failed: {str(e)[:60]}")
                    continue
                log(f"    OSM: {len(unique)} unique ways from {urllib.parse.urlparse(mirror).netloc}")
                return unique
    log("    All Overpass mirrors failed -> no roads.")
    return []


def fetch_multi_bbox_roads(bboxes, offline=False, log=print):
    """Return OSM highway 'way' elements across multiple bboxes in one Overpass
    query, cached in memory per query with single-flight de-duplication."""
    if not bboxes:
        return []

    highway_filter = 'highway~"^(motorway|trunk|primary|secondary|tertiary|unclassified|residential|service|living_street|motorway_link|trunk_link|primary_link|secondary_link|tertiary_link|road)"'
    sub_queries = [f"way({s:.6f},{w:.6f},{n:.6f},{e:.6f})[{highway_filter}];" for (s, w, n, e) in bboxes]
    query = f"[out:json][timeout:25]; ({' '.join(sub_queries)}); out geom tags;"
    ...
    with _OSM_CACHE_LOCK:
        ...
        if offline:
            return []
        event = _OSM_EVENTS.get(query)
        owner = event is None
        if owner:
            event = threading.Event()
            _OSM_EVENTS[query] = event

    if offline:
        return []

    if not owner:
        # Someone else is fetching this exact area; wait for their result.
        event.wait(timeout=40)
        with _OSM_CACHE_LOCK:
            return _OSM_CACHE.get(query, [])

    result = []
    try:
        result = _overpass_request(query, log)
    finally:
        with _OSM_CACHE_LOCK:
            # Cache only real results; don't poison the cache with an empty
            # response caused by a transient all-mirrors-down blip.
            if result:
                _OSM_CACHE[query] = result
            _OSM_EVENTS.pop(query, None)
        event.set()
    return result
...
def cluster_points_into_bboxes(points, cluster_size_m=3000.0, margin_m=BBOX_MARGIN_M):
    """
    Groups points into spatial bounding boxes of approx `cluster_size_m` (3km).
    Only creates bboxes for grid cells containing points.
    ...
    dlat_step = cluster_size_m / 111_320.0
    dlon_step = cluster_size_m / (111_320.0 * math.cos(math.radians(lat_c)))
    dlat_margin = margin_m / 111_320.0
    dlon_margin = margin_m / (111_320.0 * math.cos(math.radians(lat_c)))
    ...
    for p in points:
        lon, lat = p[0], p[1]
        gi = int((lat - min_lat) / dlat_step) if dlat_step > 0 else 0
        gj = int((lon - min_lon) / dlon_step) if dlon_step > 0 else 0
        key = (gi, gj)
        if key not in cells:
            cells[key] = []
    ...
    bboxes = []
    for (gi, gj), pts in cells.items():
        cell_lats = [pt[0] for pt in pts]
        cell_lons = [pt[1] for pt in pts]
        s = min(cell_lats) - dlat_margin
        n = max(cell_lats) + dlat_margin
        w = min(cell_lons) - dlon_margin
        e = max(cell_lons) + dlon_margin
...
def make_elements_fetcher(points, offline=False, log=print):
    """Return get_elements(lat, lon) -> OSM ways.
    Clusters points into spatial bboxes and fetches them in a single/batched Overpass query."""
    if not points:
    ...
    if max(span_ns, span_ew) <= MAX_BBOX_M:
        ...
        shared = fetch_multi_bbox_roads(
            [(min(lats) - dlat, min(lons) - dlon,
              max(lats) + dlat, max(lons) + dlon)], offline=offline, log=log)
        return lambda lat, lon: shared

    bboxes = cluster_points_into_bboxes(points, cluster_size_m=3000.0, margin_m=BBOX_MARGIN_M)
    log(f"    Points span {max(span_ns, span_ew)/1000:.1f} km -> grouped into {len(bboxes)} spatial cluster(s).")
    shared_elements = fetch_multi_bbox_roads(bboxes, offline=offline, log=log)
    return lambda lat, lon: shared_elements

def _canonical_along(tan):
    """Return the local road tangent folded to a deterministic hemisphere.

    The along-road direction read from OSM depends on the (arbitrary) order the
    way's nodes were digitised, so the same physical road can yield a tangent
    pointing either way. A road-marking image is symmetric about the road axis,
    so only that axis matters - but the raw tangent made overlays flip 0/180
    unpredictably between roads. Folding the bearing into [0, 180) (flip when it
    points into the southern/western hemisphere) makes image orientation
    reproducible and consistent, resolving the reported mis-alignment without a
    hard-coded rotation offset.
    """
    e, n = float(tan[0]), float(tan[1])
    if math.hypot(e, n) < 1e-12:
...
def _measure_road_width(frame, el, nodes_xy, elements, lat, lon, class_width):
    ...
    both sides of the road centerline, capped at the median on divided roads).
    Returns (width_m, conf) or None when it can't be trusted."""
    try:
    ...
    # Measure starting directly on the true road centerline (foot), not the offset pin
    _, _, foot, _ = _project_point_on_polyline(nodes_xy)
    foot_lon, foot_lat = frame.to_lonlat(*foot)

    bearing = road_bearing_at(nodes_xy)
    cap_l, cap_r = median_caps(frame, el, bearing, nodes_xy, elements)

    # Cap search to realistic carriageway boundary to prevent spilling into fields
    max_half = min(12.0, max(4.0, class_width * 0.75))
    cap_l = min(cap_l, max_half)
    cap_r = min(cap_r, max_half)

    try:
        res = roadwidth.measure(foot_lat, foot_lon, bearing, cap_left=cap_l, cap_right=cap_r)
    except Exception:
        return None
    if res is None:
        return None
    ...
    if conf < WIDTH_MIN_CONF:
        return None
    if not (0.5 * class_width <= total <= 1.5 * class_width):
        return None                          # detection implausible vs road class
    return total, conf


def extract_strip_count(item_name):
    """Extract strip count N from name like 'CAP PTBM 20mm X 5' -> 5 (defaults to 1)."""
    m = re.search(r"[xX]\s*(\d+)", item_name or "")
    if m:
        try:
            val = int(m.group(1))
            if val > 0:
...
def build_polygons(points, mark_length=MARK_LENGTH, offline=False,
                   ...
                   keep_points=True, doc_name="Road Markings", rate=None,
                   zone=None,
                   speed_breaker="",
                   speed_quantity=0.40,
                   speed_gap=0.60,
    ...
    applied_rate = rate if rate is not None else config.get_zone_rate(selected_zone)
    ...
    def _process_one(item):
        ...
        kml_fragment = poly_kml(pm_title, ring_lonlat_for_kml, warn=warn, description=desc)
        log_msg = (f"  [{idx}/{len(points)}] {display_name} ({name}): {hw_type} '{road_name}'  "
                   f"width={width:.1f}m ({wsrc})  dist={dist:.1f}m  "
                   f"area={area:.1f}m2{'  [curved]' if curved else ''}")

        boq_row = {
            "S.No": idx, "Placemark": display_name, "Zone": selected_zone,
        ...
        return idx, kml_fragment, boq_row, log_msg

    from concurrent.futures import ThreadPoolExecutor
    items_to_process = list(enumerate(points, 1))
    max_w = min(16, max(1, len(items_to_process)))

    with ThreadPoolExecutor(max_workers=max_w) as executor:
        results = list(executor.map(_process_one, items_to_process))

    results.sort(key=lambda r: r[0])

    for idx, kml_fragment, boq_row, log_msg in results:
        kml.append(kml_fragment)
        boq.append(boq_row)
        log(log_msg)
...
def export_excel_boq(boq, output_target):
    ...
    def _fetch_row_img(item):
        ri, row_data = item
        print(f"    Fetching satellite image for row {ri}...", flush=True)
        img = get_satellite_image(row_data["_corners"])
        return ri, img

    rows_to_fetch = list(enumerate(boq, start=2))
    images_by_row = {}
    with ThreadPoolExecutor(max_workers=min(12, max(1, len(rows_to_fetch)))) as executor:
        for ri, img in executor.map(_fetch_row_img, rows_to_fetch):
            images_by_row[ri] = img
    ...
    for ri, row_data in enumerate(boq, start=2):
        ...
        if img is not None:
            ...
            print(f"    Embedded image for row {ri}")
        else:
            ws.cell(row=ri, column=img_col, value="Image unavailable")
            print(f"    Image unavailable for row {ri}")
...
def build_image_kmz(points, doc_name="Road Markings", keep_missing=True, offline=False, mark_length=MARK_LENGTH, log=print):
    ...
    def _process_one_image_point(item_tuple):
        idx, (lon, lat, name) = item_tuple
        entry = config.resolve(name)
        ...
        if known:
            ...
            if not keep_missing:
                return idx, None, {"code": config.normalize_code(name), "item": item, "image": None, "known": False}, None
        ...
        if known and fname:
            ...
            if el is None:
                ...
                cum_s = _arc_lengths(nodes_xy)
                _, _, foot, s0 = _project_point_on_polyline(nodes_xy)
                # Local (curve-aware) road direction at the placemark's foot, not
                # a global/average road direction, so overlays on curves follow
                # the road where the point actually sits.
                _, tan = _point_and_tangent_at_s(nodes_xy, cum_s, s0)

                if MEASURE_WIDTH and not offline:
                    measured = _measure_road_width(frame, el, nodes_xy, elements, lat, lon, width)
                    if measured is not None:
                        width, conf = measured
                        wsrc = f"satellite(c={conf:.2f})"

            # Deterministic orientation: fold the tangent so the overlay aligns
            # to the road axis regardless of OSM node ordering (0/180 fix).
            tan = _canonical_along(tan)
            overlay_bearing = math.degrees(math.atan2(float(tan[0]), float(tan[1]))) % 360.0
            hw = width / 2.0

            # Compute along-road length from image aspect ratio so overlay
            # preserves the image proportions (no distortion / elongation).
            img_path = used_images.get(fname)
            if img_path:
                try:
                    from PIL import Image as _PILImage
                    with _PILImage.open(img_path) as _im:
                        iw, ih = _im.size
                    # image X → across road (width), image Y → along road (length)
                    img_aspect = iw / ih if ih > 0 else 1.0
                    overlay_length = width / img_aspect
                except Exception:
                    overlay_length = mark_length
            else:
                overlay_length = mark_length

            ring_lonlat, corners_xy = _straight_strip(
                frame, foot, tan, hw, hw, overlay_length)
        ...
        sum_item = {"code": config.normalize_code(name), "item": item,
                    "image": entry["image"] if known else None, "known": known,
                    "bearing": round(overlay_bearing, 1) if (known and fname) else None}
        log_msg = f"  [{idx}/{len(points)}] {name!r} -> {item}{'' if known else '   [UNKNOWN CODE]'}"
        return idx, kml_fragment, sum_item, log_msg
    ...
    with ThreadPoolExecutor(max_workers=max_w) as executor:
        results = list(executor.map(_process_one_image_point, items_to_process))

    results.sort(key=lambda r: r[0])

    summary = []
    for idx, kml_fragment, sum_item, log_msg in results:
        if kml_fragment is not None:
            kml.append(kml_fragment)
        summary.append(sum_item)
        if log_msg:
            log(log_msg)
...
# ==================================================================
# CORRIDOR GROUPING  ('x' placeholders delimit corridors -> Excel tabs)
# ==================================================================

def _is_corridor_delimiter(name):
    """True if a placemark name is an 'x' corridor delimiter.

    Robust to case ('x'/'X') and surrounding whitespace, and to a decorated
    delimiter like 'X - end'. The five real codes (A, B, C, D, G) never collide
    with 'x', so this is unambiguous.
    """
    if not name:
        return False
...
def has_corridor_delimiters(points):
    """True if any placemark is an 'x' corridor delimiter."""
    return any(_is_corridor_delimiter(n) for _, _, n in points)


def split_into_corridors(points):
    """Split placemarks into corridors delimited by 'x'/'X' markers.

    Every placemark BETWEEN two x markers goes into the same corridor. The x
    markers are delimiters only and are never emitted as rows. Edge cases:

      * consecutive x's                -> no empty corridor is created;
      * placemarks before the first x  -> kept as the first corridor (never
        silently discarded);
      * placemarks after the last x    -> kept as the final corridor;
      * case / whitespace variants of x -> all treated as delimiters.

    Returns an ordered list of (label, members) where members is a list of
    (orig_index, (lon, lat, name)) and labels are 'Corridor 1', 'Corridor 2', …
    """
    corridors = []
    current = []
    for idx, (lon, lat, name) in enumerate(points, 1):
        if _is_corridor_delimiter(name):
            if current:                     # close the open corridor (skip empties)
                corridors.append(current)
                current = []
            continue
        current.append((idx, (lon, lat, name)))
    if current:                             # trailing run after the last x
        corridors.append(current)
    return [(f"Corridor {i}", members) for i, members in enumerate(corridors, 1)]


def _road_width_for_point(get_elements, lon, lat, name, idx, offline=False):
    """Best available road width (m) at a placemark, plus its source, for the
    Excel BOQ. Measures from satellite imagery where possible (accurate,
    per-road), falling back to the OSM class/lane estimate, then a name/manual
    override. Returns (width_m_or_None, source_str). Never raises - any failure
    degrades to the estimate so BOQ generation can't crash on one bad point.
    """
    try:
        frame = LocalFrame(lat, lon)
        elements = get_elements(lat, lon)
    ...
    if el is None:
        try:
            width, wsrc = estimate_width({}, name, idx)
            return round(float(width), 2), f"no_road:{wsrc}"
        except Exception:
            return None, "no_road"
    ...
    try:
        tags = el.get("tags", {})
        width, wsrc = estimate_width(tags, name, idx)
        if MEASURE_WIDTH and not offline and wsrc != "provided":
            measured = _measure_road_width(frame, el, nodes_xy, elements, lat, lon, width)
            if measured is not None:
                width, conf = measured
                wsrc = f"satellite(c={conf:.2f})"
        return round(float(width), 2), wsrc
    except Exception:
        return None, "error"


def build_corridor_boq(points, zone=None, offline=False, measure=True, log=lambda *a: None):
    """
    Build the BOQ grouped into corridors delimited by 'x' placemarks (Task 5).

    Each corridor becomes one Excel tab. Within a corridor, identical items are
    grouped and counted (preserving the existing BOQ column layout) and a
    per-item **Road Width (m)** is reported - the mean of the widths measured
    (satellite-first, OSM fallback) at that item's placemarks in the corridor
    (Task 4). Returns an ordered dict: corridor_label -> list of row dicts.

    Falls back to per-item widths without measurement when `measure` is False or
    `offline` is True (keeps it network-free for tests).
    """
    selected_zone = zone or config.DEFAULT_ZONE
    zone_rate = config.get_zone_rate(selected_zone)
    corridors = split_into_corridors(points)

    # Measure width once per placemark (parallel, cached tiles make this cheap).
    get_elements = make_elements_fetcher(points, offline=offline, log=log)
    do_measure = measure and not offline

    flat = [(idx, lon, lat, name)
            for _, members in corridors for (idx, (lon, lat, name)) in members]

    widths = {}
    if flat:
        def _one(rec):
            idx, lon, lat, name = rec
            w, src = _road_width_for_point(get_elements, lon, lat, name, idx,
                                           offline=offline or not do_measure)
            return idx, w, src
        from concurrent.futures import ThreadPoolExecutor
        with ThreadPoolExecutor(max_workers=min(16, max(1, len(flat)))) as ex:
            for idx, w, src in ex.map(_one, flat):
                widths[idx] = (w, src)

    result = {}
    for label, members in corridors:
        # item -> {count, unit_cost, widths[list]}
        bucket = {}
        for idx, (lon, lat, name) in members:
            item = config.item_name(name)
            cost = config.unit_cost(name)
            if cost <= 0:
                cost = zone_rate
            w, _src = widths.get(idx, (None, ""))
            entry = bucket.setdefault(item, {"count": 0, "cost": cost, "widths": []})
            entry["count"] += 1
            if w is not None:
        ...
        for item in sorted(bucket):
            ...
            cost = e["cost"]
            ws = e["widths"]
            avg_w = round(sum(ws) / len(ws), 2) if ws else None
            rows.append({
                "Item": item,
                "Zone": selected_zone,
                "Quantity": qty,
                "Strips/Item": extract_strip_count(item),
...
def export_corridor_excel(grouped, output_target):
    """
    Write the corridor-grouped BOQ to Excel: ONE worksheet per corridor.

    Preserves the existing BOQ columns and appends a single 'Road Width (m)'
    column (Task 4/6). Sheet names are 'Corridor 1', 'Corridor 2', … validated
    and de-duplicated by _safe_sheet_title. output_target: filepath or BytesIO.
    """
    ...
    if not grouped:
        wb.create_sheet(_safe_sheet_title("Corridor 1", used_titles))

    for corridor, rows in grouped.items():
        ws = wb.create_sheet(_safe_sheet_title(corridor, used_titles))
        for ci, h in enumerate(headers, 1):
        ...
        for row in rows:
            ...
            ws.cell(row=r, column=2, value=row.get("Zone", config.DEFAULT_ZONE))
